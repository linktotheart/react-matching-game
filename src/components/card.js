...
export default function Card({data, id, handleClick}) {
	...
	return (
		...
		onClick={() => handleClick(id)} >
			<div className="front  rounded-xl  absolute bg-slate-200 flex justify-center items-center inset-0 [transform-style:preserve-3d] md:text-7xl text-6xl ">
				<span className="rounded-full leading-none  p-2 bg-slate-100">
					{data.emoji}
				</span>
			</div>
			<div className="back  rounded-xl  flex justify-center absolute inset-0 bg-pumpkin-400 items-center ">
				<span className="w-8 h-8 transform skew-x-6 rotate-45 bg-white"></span>
			</div>
		</div>
	)
}
